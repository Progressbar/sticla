...
const adminAuth = ({ key }) =>
  new Promise((resolve, reject) => (env.keyIsAdmin(key) ? resolve() : reject()));

const itemFromName = name =>
  db.data.items.find(({ name: itemName }) => name === itemName);
const orderFromToken = token =>
  db.data.orders.find(({ token: orderToken }) => token === orderToken);

const applyOp = ({
  op, reference, property, value, onDefault = () => {},
}) => {
  switch (op) {
        ...
        resolve({
          ...
          data: {
            ...
            items: db.data.items.map(({
              ...
              imageSrc,
              location: {
                storage, fridge, transit, reserved,
              },
              stockPrice,
              size,
            }) => ({
        ...
        adminAuth(body)
          .then(() => {
            resolve({
              ...
              data: {
                ...db.data,
                orders: db.data.orders.filter(Boolean).map(order => ({
                  ...order,
                  status: orderStatuses[order.status],
                })),
              },
      ...
      case 'order': {
        ...
        if (item) {
          ...
          if (Number.isNaN(amount) || amount < 1 || amount % 1 !== 0) {
            ...
            getCommandOutput(`shuf -n 3 ${__dirname}/token-words.txt | paste -sd " "`).then((tokenRaw) => {
      ...
      case 'cancel order': {
        ...
        if (order) {
          ...
          if (status <= FETCHING) {
            ...
            item.location[
              status === FETCHING ? 'transit' : 'reserved'
            ] -= amount;
            item.location[location] += amount;
      ...
      case 'admin deny order': {
        adminAuth(body)
          .then(() => {
            ...
            if (order) {
              const {
                status, name, amount, cold,
              } = order;

              const item = itemFromName(name);

              item.location[
                status === CONFIRMING ? 'reserved' : 'tranist'
              ] -= amount;
        ...
        adminAuth(body)
          .then(() => {
            ...
            switch (location) {
              ...
              case 'all orders': {
                if (property !== undefined) {
                  db.data.orders.forEach(reference =>
                    applyOp({
                      op,
                      reference,
                      property,
